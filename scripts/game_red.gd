...
@onready var soldier_preview = red_soldier_preview_scene.instantiate()
@onready var tank_preview = red_tank_preview_scene.instantiate()
@onready var radar_preview = red_radar_preview_scene.instantiate()
@onready var missile_preview = red_missile_preview_scene.instantiate()
@onready var airplane_preview = red_airplane_preview_scene.instantiate()
...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_select") and mouse_position.y > 20 and -30  < tile_position.x and tile_position.x < -9:
...
func save_pieces_to_send():
	for i in range(len(piece_positions)):
		if i <= max_red_soldiers:
			red_soldier_positions = []
			red_soldier_positions.append(piece_positions[i]) 
		if i > max_red_soldiers and i <= max_red_tanks:
			red_tank_positions = []
			red_tank_positions.append(piece_positions[i]) 
		if i > max_red_tanks and i <= max_red_radars:
			red_radar_positions = []
			red_radar_positions.append(piece_positions[i])
		if i > max_red_radars and i <= max_red_missiles:
			red_missile_positions = []
			red_missile_positions.append(piece_positions[i])
		if i > max_red_missiles and i <= max_red_airplanes:
			red_airplane_positions = []
			red_airplane_positions.append(piece_positions[i])  
	return 
